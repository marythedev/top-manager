...
router.post("/update", checkAuthorization, (req, res) => {
    db_prj.updateProject(req.body)
        ...
        .catch(() => { 
            res.status(500).send("Problem encountered while updating project data. Try again later.");
        })
...
router.get("/delete/:prjNumber", checkAuthorization, (req, res) => {
    db_prj.deleteProject(req.params.prjNumber)
        .then(() => {
            ...
            res.status(500).send("Problem encountered while deleting project data. Try again later.");
        });
});
